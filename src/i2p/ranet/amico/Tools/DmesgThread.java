...
public class DmesgThread {
	private static final String TAG="DmesgThread";
	...
	private static final String logPath= Environment.getExternalStorageDirectory().getPath();
	...
	private Runnable mLogCat = new Runnable()
	{
		public void run()
		{
			if(mThread.isAlive()==true)
			{
				...
				Toast.makeText(mContext, "Get dmesg was done.", Toast.LENGTH_LONG).show();
			}
		}
	};
	...
	public DmesgThread(Context c) 
	{
		...
		int count=0;
		File file=new File(logPath+YEAR+MONTH+DAY+".dmesg");
		while(file.exists()==true)
		{
			count+=1;
			file=new File(logPath+YEAR+MONTH+DAY+"-"+count+".dmesg");//ex:/sdcard/20120709-1.log
		}
		logFile=file;
